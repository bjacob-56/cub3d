...
/*   Updated: 2020/12/10 10:51:16 by bjacob           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
t_image	ft_display_stripes(t_session t_ses, t_window t_win, t_player player, t_image t_img, t_ray ray)
{
	...
	i = 0;
	sort_sprites(t_win.map_info.sprites, t_win.map_info.nb_sprites, player); // tab** ?
...
//	dprintf(1, "sp_x = %f, sp_y = %f\n", t_win.map_info.sprites[i].x, t_win.map_info.sprites[i].y);
//	dprintf(1, "play_x = %f, play_y = %f\n", player.pos_x, player.pos_y);
...
//				dprintf(1, "transY = %f\n", transY);
		...
		int	sprite_screenX = (int)(t_win.x_w * (1 + transX / transY)); // a confirmer
//		int	sprite_screenX = (int)(t_img_s.x_i * (1 + transX / transY)); // a confirmer

		int	sprite_height = abs((int)(t_win.y_w / transY)); // a confirmer
		int	sprite_width = abs((int)(t_win.y_w / transY)); // a confirmer

		int	drawStartY = ft_max((t_win.y_w - sprite_height) / 2, 0);
		int	drawEndY = ft_min((t_win.y_w + sprite_height) / 2, t_win.y_w - 1);
	//	int	drawStartX = ft_max((sprite_screenX - sprite_width) / 2, 0);
		int	drawStartX = (sprite_screenX - sprite_width) / 2;
		int	drawEndX = ft_min((sprite_screenX + sprite_width) / 2, t_win.x_w - 1);

		dprintf(1, "drawStartX = %d\n", drawStartX);
		dprintf(1, "drawEndX = %d\n", drawEndX);
		...
		while (j < drawEndX)
		{
			int texX = (int)(256 * (j - drawStartX) * t_img_s.x_i / sprite_width) / 256; // 800 a ajuster
			int y;
			y = drawStartY;
			if (transY > 0 && j > 0 && j < t_win.x_w && transY < t_win.z_dist[j]) // a confirmer
			{
				while (y < drawEndY)
				{
					int	d;
